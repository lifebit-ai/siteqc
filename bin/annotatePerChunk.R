...
##INFO
# all files are explicitly passed to this script
# final and king are pulled from global env
# Setup -------------------------------------------------------------------
...
# Really it's if not one, then the other. 
if((!king & !final) | (is.null(king) & is.null(king))){
  stop('Please set either king or final as T')
}
...
cat('Generating output statistics...')
...
if(final){
  ...
  outfile <- file.path(outpath, paste0(Sys.Date(),
                                       'SUBCOL_Summary_stats_for_plotting.txt'))
}
